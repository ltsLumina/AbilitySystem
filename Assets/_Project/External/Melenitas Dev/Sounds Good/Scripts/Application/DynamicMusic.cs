...
{
	SourcePoolElement referenceSourcePoolElement;
	Dictionary<string, SourcePoolElement> sourcePoolElementDictionary = new ();

	Dictionary<string, float> volumeDictionary = new ();
	Dictionary<string, Vector2> hearDistanceDictionary = new ();
	Vector2 defaultHearDistance = new (3, 500);
	float pitch = 1;
	Vector2 pitchRange = new (0.85f, 1.15f);
	string id;
...
public partial class DynamicMusic // Fields (Callbacks)
{
	Action onPlay;
	Action onComplete;
	Action onLoopCycleComplete;
	Action onPause;
	Action onPauseComplete;
	Action onResume;
}
...
public partial class DynamicMusic // Properties
{
    ...
    /// <summary>
    ///     Duration in seconds of matched clip (use the first clip of the array because they should have the same
    ///     duration).
    /// </summary>
...
public partial class DynamicMusic // Public Methods
{
	...
	{
		foreach (KeyValuePair<string, SourcePoolElement> tagSourcePair in sourcePoolElementDictionary)
		{
    ...
    public DynamicMusic SetTrackVolume(Track track, float volume, Vector2 hearDistance) => SetTrackVolume(track.ToString(), volume, hearDistance);
    ...
    /// </summary>
    /// <param name="output">
    ///     Output you've created before inside Master AudioMixer
    ///     (Remember reload the outputs database on Output Manager Window)
    /// </param>
    ...
    public void Play(float fadeInTime = 0)
	{
		...
		for (int i = 0; i < sourcePoolElementDictionary.Count; i++)
		{
			...
			if (i == 0)
			{
				...
				referenceSourcePoolElement.SetVolume(volumeDictionary[tag], hearDistanceDictionary[tag]).SetPitch(pitch).SetLoop(loop).SetClip(Clips[i]).SetPosition(position).SetFollowTarget(followTarget).SetSpatialSound(spatialSound).SetFadeOut(fadeOutTime).SetId(id).SetOutput
						(output).OnPlay(onPlay).OnComplete(onComplete).OnLoopCycleComplete(onLoopCycleComplete).OnPause(onPause).OnPauseComplete(onPauseComplete).OnResume(onResume).Play(fadeInTime);

				continue;
			}

			newSourcePoolElement.SetVolume(volumeDictionary[tag], hearDistanceDictionary[tag]).SetPitch(pitch).SetLoop(loop).SetClip(Clips[i]).SetPosition(position).SetFollowTarget(followTarget).SetSpatialSound(spatialSound).SetFadeOut(fadeOutTime).SetId(id).SetOutput
					(output).Play(fadeInTime);
		}
	}
    ...
    public void Pause(float fadeOutTime = 0)
	{
		...
		foreach (SourcePoolElement source in sourcePoolElementDictionary.Values) source.Pause(fadeOutTime);
	}
    ...
    public void Resume(float fadeInTime = 0)
	{
		...
		foreach (SourcePoolElement source in sourcePoolElementDictionary.Values) source.Resume(fadeInTime);
	}
