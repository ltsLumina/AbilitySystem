...
public partial class SourcePoolElement // Fields
{
	...
	float playingTimeForNextSong;
	float playlistDuration;
	AudioClip cachedClipOnPause;

	Coroutine lerpVolumeCor;
	Coroutine fadeInOnChangeTrackCor;

	SourceState currentState = SourceState.Stopped;
...
public partial class SourcePoolElement // Private Methods
{
	...
	void PlayNextSong(bool firstTrack = false)
	{
		if (playlist.Count == 0) return;
	...
	void ChangeState(SourceState newState) => currentState = newState;
}

public partial class SourcePoolElement // Private Methods (Coroutines)
{
	IEnumerator LerpVolume(float newVolume, float lerpTime, Action onFinishLerp = null, bool ignorePause = false)
	{
