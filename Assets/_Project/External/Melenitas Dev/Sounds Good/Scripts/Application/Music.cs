...
public partial class Music // Fields
{
	...
	bool spatialSound;
	float fadeOutTime;
	bool randomClip = true;
	bool forgetSourcePoolOnStop;
	AudioMixerGroup output;
	string cachedSoundTag;
}
...
public partial class Music // Public Methods
{
    ...
    public void Play(float fadeInTime = 0)
	{
		...
		sourcePoolElement.SetVolume(Volume, hearDistance).SetPitch(pitch).SetLoop(loop).SetClip(Clip).SetPosition(position).SetFollowTarget(followTarget).SetSpatialSound(spatialSound).SetFadeOut(fadeOutTime).SetId(id).SetOutput(output).OnPlay(onPlay).OnComplete
				(onComplete).OnLoopCycleComplete(onLoopCycleComplete).OnPause(onPause).OnPauseComplete(onPauseComplete).OnResume(onResume).Play(fadeInTime);
	}
