...
{
	SourcePoolElement sourcePoolElement;

	Vector2 hearDistance = new (3, 500);
	float pitch = 1;
...
public partial class Playlist // Public Methods
{
    ...
    public void Play()
	{
		...
		sourcePoolElement.MarkAsPlaylist().SetVolume(Volume, hearDistance).SetPitch(pitch).SetLoop(loop).SetPlaylist(playlist).SetPosition(position).SetFollowTarget(followTarget).SetSpatialSound(spatialSound).SetFadeIn(fadeInTime).SetFadeOut(fadeOutTime).SetId(id).SetOutput
				(output).OnPlay(onPlay).OnComplete(onComplete).OnLoopCycleComplete(onLoopCycleComplete).OnNextTrackStart(onNextTrackStart).OnPause(onPause).OnPauseComplete(onPauseComplete).OnResume(onResume).PlayPlaylist(fadeInTime);
	}
